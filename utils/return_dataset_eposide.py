...
def return_dataset(args,use_test_loader=True):
    ...
    image_set_file_unl = \
        os.path.join(base_path,
                     ...
                     args.target + '_%d.txt' % (args.num))
    image_set_file_dict=np.load('./peusde_multi_3shot/'+'No_change_'+args.source+'_'+args.target+'_unlabel_3shot_stage1.npy', allow_pickle=True).item()
    #image_set_file_dict=np.load('./peusde/'+'No_change_'+args.source+'_'+args.target+'_unlabel_3shot_stage2.npy', allow_pickle=True).item()
    #image_set_file_dict=np.load('./peusde_stage2/'+'No_change_'+args.source+'_'+args.target+'_unlabel_3shot_stage1.npy', allow_pickle=True).item()
    #print(type(image_set_file_dict))
    #for key in image_set_file_dict.keys():
        #print(key)
    #exit()
    #print(image_set_file_s)
    #print(image_set_file_t)
    #print(image_set_file_t_val)
    #print(image_set_file_unl)
    #exit()
    if args.net == 'alexnet':
        crop_size = 227
    else:
    ...
    source_dataset = Imagelists_VISDA_eposide(image_set_file_s,image_set_file_t,image_set_file_unl,image_set_file_dict, root=root,
                                      transform=data_transforms['train'],target_transform=data_transforms['train'])
