...
parser.add_argument('--NCE_weight', type=float, default=0.02, help='NCE_weight')
...
if NCE_loss:
    record_dir = '/gdata2/tuky/DA_BS/%s_randomsamples/%skper/%s_shot/%s/NCE_weight_%s/%s' % (args.threshold, args.k_per, args.num, args.dataset,  args.NCE_weight, args.method)+localtime+'_'+args.source+'_'+args.target+'_unlabel_NCE_'+str(unlabel_NCE_weight)
if B_Twin_loss:
...
def train(num_instance_2=12):
    ...
    for step in range(all_step):
        ...
        #print(ranmdom_fuse)
        #exit()
        #print(im_data_t.shape)
        #exit()
        #print(im_data_tu.shape)
        #exit()
        if True:
            #print(step)
            if ranmdom_fuse>0.5:
        ...
        #print(gt_labels_s.shape)
        #exit()
        target_3 = torch.cat((gt_labels_s[:num_instance],gt_labels_s, gt_labels_t), 0)        
        output = G(data)
        source_mix=False
        #print(output.shape)
        #print(im_data_s.shape,im_data_t.shape,im_data_tu_48_simple.shape,im_data_tu_48_complex.shape,im_data_tu.shape)
        #exit()
        if source_mix:
            if ranmdom_fuse>0.7:
        ...
        target_complex=out1[num_instance+24:48+num_instance]
            ...
            #print(source_complex_soft.shape)
            #exit()
        Fixmatch=True
        if Fixmatch:
        ...
        if False:
            ...
            if B_Twin_loss:
                ...
                if False:
                    c = bn(f_a).T @ bn(f_b)
                    #print((f_a_norm @ f_b_norm).shape)
                    #print(torch.mm(f_a_norm.transpose(0,1).contiguous(),f_b_norm).shape)
                    #print((( f_a_norm.transpose(0,1).contiguous() @ f_b_norm)-torch.mm(f_a_norm.transpose(0,1).contiguous(),f_b_norm)).abs().sum())
                    #exit()
                #print(c.shape)
                    c.div_(24)
