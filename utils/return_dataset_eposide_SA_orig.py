...
def return_dataset(args, use_test_loader=True):
    ...
    image_set_file_dict = np.load('./peusde_multi_baseline_alexnet_'+str(args.num)+'shot/' + args.source + '_' + args.target + '_unlabel_'+str(args.num)+'shot_stage1.npy', allow_pickle=True).item()
    # image_set_file_dict = np.load('./peusde_multi_baseline_alexnet_3shot/'+args.source+'_'+args.target+'_unlabel_3shot_stage1.npy', allow_pickle=True).item()
    ...
    k_per = args.k_per
    source_dataset = Imagelists_VISDA_eposide_two_unlabel(k_per, bs, image_set_file_s,image_set_file_t,image_set_file_unl,image_set_file_dict, root=root,
                                      transform=data_transforms['train'],target_transform=data_transforms['train'],unlabel_transform_1=data_transforms['val'],unlabel_transform_2=data_transforms['self'])
    ...
    target_dataset_val = Imagelists_VISDA(image_set_file_t_val, root=root,
                                          transform=data_transforms['val'])
    target_dataset_unl = Imagelists_VISDA_2_augment(image_set_file_unl, root=root,
                                          transform=data_transforms['val'],transform_2=data_transforms['self'])
    if use_test_loader:
